...
from backend.modules.regulation_fragment.application.dto.create_regulation_fragment_dto import \
...
def get_all():
    return [fragment.model_dump() for fragment in container.regulation_fragment_service().findAll()]


@regulation_fragment_controller.get('/regulation-fragments/<fragment_id>')
def get_by_id(fragment_id: str):
    return container.regulation_fragment_service().findById(int(fragment_id)).model_dump()
