...
import { ExamplesStateEntry, useRemoveExample } from '@/hooks/useExamplesStore';
import {
  Select,
  SelectContent,
  SelectItem,
...
interface Props {
  className?: string;
  example: ExamplesStateEntry;
}

export function Example({ className, example }: Readonly<Props>) {
  const [selectedFragmentId] = useSelectedRegulationFragmentId();
  const { data: atoms = [] } = useAtoms(selectedFragmentId);
  const removeExample = useRemoveExample(selectedFragmentId);
    ...
    <div className={cn('overflow-y-auto pb-24 size-full', className)}>
      ...
      <div className="px-4">
        <div className={'flex flex-row gap-2 items-center justify-start mb-2'}>
          <h4 className="font-medium">Prolog Atoms</h4>
          <ConfirmDeleteDialog
            title="Example"
            description="Are you sure you want to delete this example? This action cannot be undone."
            isPending={false}
            isError={false}
            onDelete={() => removeExample(example.key)}
            errorMessage="Failed to delete the example. Please try again."
          />
